...
def test_video(args):
    ...
    with torch.no_grad():
        while cap.isOpened():
            ...
            disp_gray = (disp_resized - disp_resized.min()) / (disp_resized.max() - disp_resized.min())
            disp_gray = (disp_gray * 255).astype(np.uint8)

            

            # Blend the grayscale depth map with the frame
            blended = cv2.addWeighted(frame, 0.6, cv2.cvtColor(disp_gray, cv2.COLOR_GRAY2BGR), 0.4, 0)
